...
- **头部**：姓名、所属站点、本期服务次数
- **得分概览**：S / A / B / C / D 当期均分，用 5 个数字卡片展示
- **趋势图**：过去 12 个周期的 S 分折线图（recharts `LineChart`），hover 显示数值
- **ABCD 雷达图**（可选）：4 维度的当期均分雷达图，直观看强弱项
- **底部关闭按钮**
...
| 文件 | 变更 |
|------|------|
| `src/quality/QualityPage.tsx` | 重写 DashboardView：替换现有 KPI/表格为员工质量评分 + 站点对比 |
| `src/quality/quality.css` | 新增样式：员工表格、详情 Modal 趋势图容器、周/月 toggle、色标 |
