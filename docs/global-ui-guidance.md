...
| `--info-bg` | `#ECFEFF` | 信息状态背景（进行中等） |
| `--info-text` | `#0891B2` | 信息状态文字 |
...
所有角色页面（质量管理、站点运营）共享统一的 shell 布局：

| 元素 | 规格 | 说明 |
|------|------|------|
| 顶栏高度 | 64px | 纯白背景 #FFFFFF + 1px #E5E7EB 下边框 |
| 侧栏宽度 | 56px | 纯白背景 #FFFFFF + 1px #E5E7EB 右边框 |
| Logo 图标 | 36×36px, 10px 圆角 | 蓝底白色图标 |
| 状态点 | 6px, #116B4C | 表示运行状态 |
| Grid 布局 | 2 行: 64px + 1fr | 顶栏跨所有列 (grid-column: 1 / -1) |
| Copilot 面板 | grid-row: 2 | 仅在内容行显示，不覆盖顶栏 |

顶栏结构：左侧（Logo + 标题 + 状态）→ 右侧（操作按钮区）
...
- 用于实体状态，不可点击。
- 使用语义色：success（`--success-bg/text`）/ warning（`--warning-bg/text`）/ info（`--info-bg/text`）/ muted（`--muted-bg/text`）。
...
- Table container：白底 + `--line` 边框 + `--radius-lg` 圆角，自然高度随内容增长（不使用 `flex: 1` / `overflow-y: auto` 限定高度），滚动发生在页面级别。
...
tab 内容区 (scrollable):
  ...
  chat bubbles:
    [worker avatar] speaker + timestamp
                    white bubble (left-aligned)
    
                    speaker + timestamp [object avatar]
                    green bubble (right-aligned)
  download buttons (audio + transcript .txt)
```

规则：

- 服务人员（`social_worker`）：左侧对齐，白色气泡（`#FFFFFF`），蓝色 avatar（首字母 + `#3B82F6` 底）
- 服务对象（`service_object`）：右侧对齐，绿色气泡（`#DCF8C6`），绿色 avatar（首字母 + `#22C55E` 底）
- 每条气泡显示 avatar + 说话人标签 `speakerLabel` + 时间戳（`startSecond` 格式化为 `MM:SS`）+ 对话文本
- Avatar 32px 圆形，显示说话人首字母
- 气泡圆角 12px，padding 10px 14px
- 对话按时间顺序排列
- 无对话数据时不显示聊天区域
